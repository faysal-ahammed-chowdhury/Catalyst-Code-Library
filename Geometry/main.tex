...
\section*{C-Hash for Pairs}

\begin{minted}[fontsize=\scriptsize, breaklines]{cpp}
uint64_t x = splitmix64(p.first + FIXED_RANDOM);
uint64_t y = splitmix64(p.second + FIXED_RANDOM);
return splitmix64(x ^ (y << 1));
// another way (faster)
uint64_t x = ((uint64_t)p.first << 32) ^ (uint64_t)p.second;
return splitmix64(x + FIXED_RANDOM);
// for tuple
uint64_t x = splitmix64(a + FIXED_RANDOM);
uint64_t y = splitmix64(b + FIXED_RANDOM);
uint64_t z = splitmix64(c + FIXED_RANDOM);
return splitmix64(x ^ (y << 1) ^ (z << 2));
